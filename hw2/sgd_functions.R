...
sgd = function(X, Y, m = 1, iter = 1000, step = 0.01, alpha=0.9, detail=F) {
  ...
  # glmBetas = as.numeric(coef(glm(Y ~ X + 0, family = "binomial")))
  # plot(glmBetas ~ beta, asp = 1)
  # abline(0, 1)
  # abline(lm(glmBetas ~ beta), col = "red")
  # Plotting
  plotData = data.frame(iteration = 1:iter,
                        logLike = -logLike,
                        windowLogLike = -windowLogLike,
                        weightedLogLike = -weightedLogLike,
                        data = -contributions)
  g = ggplot(plotData)
  g = g + geom_line(aes(iteration, logLike))
  g = g + geom_line(aes(iteration, windowLogLike), col = "green")
  g = g + geom_line(aes(iteration, weightedLogLike), col = "red")
  if (detail) {
    g = g + geom_point(aes(iteration, data), col = "blue", alpha = 0.1)
    g = g + ylim(c(min(-windowLogLike, -weightedLogLike, -logLike),
           max(-windowLogLike, -weightedLogLike, -logLike)))
  }
  return(g)
}
...
# Part D
########################################
sgd_robbinsMonro = function(X, Y, m = 1, iter = 1000,
               C = 1, t0 = 1, a = 0.75,
               alpha=0.9, detail=F) {
  N = nrow(X)
  P = ncol(X)
